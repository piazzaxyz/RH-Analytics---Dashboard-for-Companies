...
def calculate_payroll(employee_id: int, month: str, db: Session):
    ...
    timesheets = (
        db.query(Timesheet).filter(Timesheet.employee_id == employee_id, Timesheet.date.like(f"{month}-%")).all()
    )
    ...
    gross_salary = (
        base_salary + overtime_50 + overtime_100 + night_additional + dsr + bonus + hazard_pay + unhealthy_pay
    )
    ...
    loan_ids = [loan.id for loan in db.query(Loan).filter(Loan.employee_id == employee_id).all()]
    loan_discount = (
        sum(
            [
                li.amount
                for li in db.query(LoanInstallment)
                .filter(
