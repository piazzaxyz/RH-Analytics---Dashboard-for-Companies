...
def list_payrolls(
    ...
    emp_names = (
        {e.id: e.full_name for e in db.query(Employee).filter(Employee.id.in_(emp_ids)).all()} if emp_ids else {}
    )
    return [
        PayrollResponse(
            id=p.id,
            employee_id=p.employee_id,
            employee_name=emp_names.get(p.employee_id, ""),
            reference_month=p.reference_month,
            base_salary=p.base_salary,
            overtime_50_value=p.overtime_50_value,
            overtime_100_value=p.overtime_100_value,
            night_additional_value=p.night_additional_value,
            dsr_value=p.dsr_value,
            bonus_value=p.bonus_value,
            hazard_pay=p.hazard_pay,
            unhealthy_pay=p.unhealthy_pay,
            gross_salary=p.gross_salary,
            inss_value=p.inss_value,
            irrf_value=p.irrf_value,
            vt_discount=p.vt_discount,
            vr_discount=p.vr_discount,
            health_plan_discount=p.health_plan_discount,
            loan_discount=p.loan_discount,
            absence_discount=p.absence_discount,
            alimony_discount=p.alimony_discount,
            other_discounts=p.other_discounts,
...
def calculate_individual_payroll(
    employee_id: int, month: str, db: Session = Depends(get_db), user: User = Depends(require_role("admin", "rh"))
):
...
def calculate_batch_payroll(
    month: str, db: Session = Depends(get_db), user: User = Depends(require_role("admin", "rh"))
):
    try:
...
def payroll_summary(
    ...
    for dept in departments:
        total = (
            db.query(Payroll)
            .filter(
                Payroll.reference_month == month,
                Payroll.employee_id.in_(db.query(Employee.id).filter(Employee.department_id == dept.id)),
            )
            .with_entities(Payroll.gross_salary)
            .all()
        )
        total_gross = sum([t[0] for t in total])
        summary.append({"department_id": dept.id, "department_name": dept.name, "total_gross_salary": total_gross})
    return summary


@router.patch("/{id}/bonus", response_model=PayrollResponse)
def update_payroll_bonus(
    ...
    payroll.gross_salary = (
        payroll.base_salary
        + payroll.overtime_50_value
        + payroll.overtime_100_value
        + payroll.night_additional_value
        + payroll.dsr_value
        + bonus
        + payroll.hazard_pay
        + payroll.unhealthy_pay
    )
    ...
    payroll.net_salary = (
        ...
        - payroll.vt_discount
        - payroll.vr_discount
        - payroll.health_plan_discount
        - payroll.loan_discount
        - payroll.absence_discount
        - payroll.alimony_discount
