...
from app.schemas.dashboard import (
    ...
    OvertimeByDepartmentResponse,
    PayrollEvolutionResponse,
)
...
def overview(
    db: Session = Depends(get_db), user: User = Depends(require_role("admin", "gestor", "rh", "visualizador"))
):
    headcount = db.query(Employee).filter(Employee.status == EmployeeStatus.ativo).count()
    total_payroll = (
        db.query(Payroll).filter(Payroll.status == PayrollStatus.processado).with_entities(Payroll.net_salary).all()
    )
...
def payroll_evolution(
    ...
    for i in range(12):
        ...
        total = (
            db.query(Payroll)
            .filter(Payroll.reference_month == month, Payroll.status == PayrollStatus.processado)
            .with_entities(Payroll.net_salary)
            .all()
        )
...
def headcount_by_department(
    ...
    for dept in departments:
        count = (
            db.query(Employee)
            .filter(Employee.department_id == dept.id, Employee.status == EmployeeStatus.ativo)
            .count()
        )
        result.append(HeadcountByDepartmentResponse(department_id=dept.id, department_name=dept.name, headcount=count))
    return result
...
def overtime_by_department(
    ...
    try:
        ...
        for dept in departments:
            ...
            for e in employees:
                timesheets = (
                    db.query(Timesheet).filter(Timesheet.employee_id == e.id, Timesheet.date.like(f"{month}-%")).all()
                )
                for t in timesheets:
                    overtime += (t.overtime_50_minutes or 0) + (t.overtime_100_minutes or 0)
            result.append(
                OvertimeByDepartmentResponse(
                    department_id=dept.id, department_name=dept.name, overtime_hours=overtime / 60.0
                )
            )
        return result
    except Exception:
