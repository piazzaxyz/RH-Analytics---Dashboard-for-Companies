...
def list_evaluations(
    ...
    evals = query.all()
    emp_names = (
        {e.id: e.full_name for e in db.query(Employee).filter(Employee.id.in_([x.employee_id for x in evals])).all()}
        if evals
        else {}
    )
    return [
...
def update_evaluation(
    id: int, data: dict, db: Session = Depends(get_db), user: User = Depends(require_role("admin", "rh"))
):
    e = db.query(Evaluation).filter(Evaluation.id == id).first()
    if not e:
        raise HTTPException(status_code=404, detail="Avaliação não encontrada")
