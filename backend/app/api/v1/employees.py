...
def create_employee(
    ...
    supervisor = (
        db.query(Employee).filter(Employee.id == employee.supervisor_id).first() if employee.supervisor_id else None
    )
...
def _employee_to_response(e, db: Session) -> EmployeeResponse:
    ...
    return EmployeeResponse(
        ...
        bank_account=e.bank_account,
        bank_account_type=e.bank_account_type,
        photo_url=f"/api/v1/employees/{e.id}/photo" if e.photo_blob else None,
        salary=e.salary,
        pis_pasep=e.pis_pasep,
        created_at=e.created_at,
        updated_at=e.updated_at,
...
def upload_document(
    ...
    blob = file.file.read()
    doc = Document(
        employee_id=id, type=doc_type, filename=file.filename or "file", file_blob=blob.hex(), uploaded_by=user.id
    )
    db.add(doc)
    db.commit()
    db.refresh(doc)
...
def list_documents(
    id: int, db: Session = Depends(get_db), user: User = Depends(require_role("admin", "gestor", "rh", "visualizador"))
):
