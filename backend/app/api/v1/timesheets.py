...
from app.schemas.timesheet import TimesheetImportResponse, TimesheetResponse
...
def import_timesheets(
    ...
    db.commit()
    fmt = (
        TimesheetImportFormat.csv
        if format == "csv"
        else (TimesheetImportFormat.json if format == "json" else TimesheetImportFormat.pdf)
    )
    imp = TimesheetImport(
        imported_by=user.id,
    ...
    db.refresh(imp)
    log_audit(
        db, user.id, "import_timesheets", "timesheet_import", imp.id, None, {"filename": file.filename}, "127.0.0.1"
    )
